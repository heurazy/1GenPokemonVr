...
local WANT_IDS = { "textSpeed", "animations", "battleStyle", "ruleset",
                   ...
                   "speed", "mods", "controls" }
check(#om.rows == #WANT_IDS,
  "vanilla options row count (plus MODS/CONTROLS)")
for i, id in ipairs(WANT_IDS) do
  check(om.rows[i].id == id, "options row order: " .. id)
end

local pointerOptionGame = optGame()
local pointerOptionMenu = OptionsMenu.new(pointerOptionGame)
pointerOptionMenu:onVRPointerClick(20, 16)
check(pointerOptionGame.save.options.textSpeed == 5,
  "one laser click advances the main OPTION row exactly once")
...
-- VR OPTION is a dedicated START-menu category immediately below OPTION,
-- not another row in the already crowded ordinary options list.
local vrGame = optGame()
vrGame.save.options.vrCamera = "immersive"
vrGame.save.options.vrWorldScale = 64
vrGame.save.options.vrTurnMode = "snap"
vrGame.save.options.vrSnapAngle = 30
vrGame.save.flags = { EVENT_GOT_POKEDEX = true }
vrGame.save.party = { {} }
vrGame.save.player = { name = "RED" }
vrGame.save.pokedex = { owned = {} }
local OpenXR = require("src.vr.OpenXR")
OpenXR.configure({ "--vr" })
local vrStart = StartMenu.new(vrGame)
local optionIndex
for i, item in ipairs(vrStart.items) do
  if item.label == "OPTION" then optionIndex = i end
end
check(optionIndex ~= nil
  and vrStart.items[optionIndex + 1].label == "VR OPTION",
  "VR OPTION appears directly below OPTION in the START menu")
vrStart.items[optionIndex + 1].onSelect()
local vrm = vrGame.stack:top()
local VROptionsMenu = require("src.ui.VROptionsMenu")
check(getmetatable(vrm) == VROptionsMenu,
  "VR OPTION opens its dedicated category")
check(vrm.rows[1].value(vrGame) == "IMMERSIVE",
  "the current/original camera is the default")
vrm.rows[1].step(vrGame, 1)
check(vrGame.save.options.vrCamera == "orbit",
  "camera cycles to Animal Crossing-style ORBIT")
vrm.rows[1].step(vrGame, 1)
check(vrGame.save.options.vrCamera == "overhead",
  "camera cycles to OVERHEAD")
vrm.rows[1].step(vrGame, 1)
check(vrGame.save.options.vrCamera == "first_person",
  "camera cycles to FIRST PERSON")

-- Standalone Quest exposes only settings which the current first-person
-- renderer actually consumes. Legacy camera/distance/pitch rows stay on PCVR.
OpenXR._questBuild = true
vrGame.save.options.vrPlayerHeight = 130
vrGame.save.options.vrRenderScale = 75
vrGame.save.options.vrRenderDistance = 28
vrGame.save.options.vrRefreshRate = 90
vrGame.save.options.vrShadows = false
vrGame.save.options.vrWorldScale = 64
local questMenu = VROptionsMenu.new(vrGame)
local questRows = {}
for _, row in ipairs(questMenu.rows) do questRows[row.id] = row end
check(questMenu.rows[1].id == "vrMode"
  and questMenu.rows[1].value(vrGame) == "FIRST PERSON"
  and questMenu.rows[1].step == nil,
  "Quest reports the locked first-person mode instead of a dead camera cycler")
check(questRows.vrCameraDistance == nil and questRows.vrCameraPitch == nil
  and questRows.vrCameraOffsetX == nil and questRows.vrHeadTracking == nil,
  "Quest hides legacy camera rows which no longer affect its renderer")
questRows.vrPlayerHeight.step(vrGame, 1)
check(vrGame.save.options.vrPlayerHeight == 140
  and questRows.vrPlayerHeight.value(vrGame) == "140 CM",
  "Quest player-height calibration is live and clearly labelled")
questRows.vrRenderScale.step(vrGame, 1)
check(vrGame.save.options.vrRenderScale == 85
  and questRows.vrRenderScale.value(vrGame) == "85%",
  "Quest render quality cycles through the active stereo resolution scales")
check(questRows.vrRenderDistance.value(vrGame) == "28 M",
  "Quest exposes the expanded terrain horizon")
questRows.vrRenderDistance.step(vrGame, 1)
check(vrGame.save.options.vrRenderDistance == 32
  and questRows.vrRenderDistance.value(vrGame) == "32 M",
  "Quest draw distance remains adjustable without changing world scale")
check(questRows.vrRefreshRate.value(vrGame) == "90 HZ",
  "Quest defaults to a headset-paced 90 Hz target")
questRows.vrRefreshRate.step(vrGame, 1)
check(vrGame.save.options.vrRefreshRate == 120
  and questRows.vrRefreshRate.value(vrGame) == "120 HZ",
  "Quest display-rate target cycles independently of the 60 Hz logic clock")
check(questRows.vrShadows.value(vrGame) == "OFF",
  "Quest shadows are disabled in the default performance profile")
questRows.vrShadows.step(vrGame, 1)
check(vrGame.save.options.vrShadows == true
  and questRows.vrShadows.value(vrGame) == "ON",
  "Quest shadows remain an explicit opt-in from the VR menu")
check(questRows.vrFloorLevel.value(vrGame) == "AUTO LEVEL"
  and questRows.vrController.value(vrGame) == "QUEST TOUCH",
  "Quest menu surfaces automatic floor leveling and the active controllers")
OpenXR._questBuild = false
OpenXR.configure({ "--no-vr" })
