...
-- Full ray support for every manager screen that owns a vertical list. This
-- also makes the common VR input layer suppress stick navigation here.
function ManagerState:onVRPointer(_, y)
  if self.overlay then
    if self.overlay.kind == "confirm" then
      self.overlay.index = y < 88 and 1 or 2
    end
    return true
  end
  if self.screen == "options" then
    ...
    if slot >= 1 and slot <= OptionRows.VISIBLE
        and (self.optionRows or {})[index] then self.cursor = index end
    return true
  end
  local rows = self:rowsForScreen()
  if self.screen == "detail" then
    local index = math.floor(y / 8) - 10
    if rows[index] then self.cursor = index end
    return true
  elseif self.screen == "apply" then
    local index = math.floor(y / 8) - 11
    if rows[index] then self.cursor = index end
    return true
  end
  local slot = math.floor(y / 8) - LIST_TOP + 1
  local index = (self.scroll or 1) + slot - 1
  if slot >= 1 and slot <= LIST_ROWS and rows[index]
      and not rows[index].header then self.cursor = index end
  return true
end

function ManagerState:vrScrollMetrics()
  if self.overlay then return 0, 0, 1 end
  if self.screen == "options" then
    return self.scroll or 0, #(self.optionRows or {}), OptionRows.VISIBLE
  end
  if self.screen == "detail" then
    local m = self.currentMod
    local lines = wrap(m and (m.error and ("FAILED: " .. m.error)
                       or m.description) or "", 16)
    return math.max(0, (self.descScroll or 1) - 1), #lines, 5
  end
  if self.screen == "apply" then return 0, 0, 1 end
  return math.max(0, (self.scroll or 1) - 1),
         #self:rowsForScreen(), LIST_ROWS
end

function ManagerState:vrSetScroll(value)
  local scroll, total, visible = self:vrScrollMetrics()
  local target = math.max(0, math.min(math.max(0, total - visible),
                                      tonumber(value) or scroll))
  if self.screen == "options" then
    self.scroll = target
    self.cursor = math.max(target + 1,
      math.min(target + visible, self.cursor))
  elseif self.screen == "detail" then
    self.descScroll = target + 1
  elseif self.screen ~= "apply" and not self.overlay then
    self.scroll = target + 1
    local rows = self:rowsForScreen()
    local first, last = self.scroll,
      math.min(#rows, self.scroll + visible - 1)
    if self.cursor < first or self.cursor > last or
        (rows[self.cursor] and rows[self.cursor].header) then
      for i = first, last do
        if rows[i] and not rows[i].header then self.cursor = i break end
      end
    end
  end
end
...
function ManagerState:drawRows(rows)
  ...
  VRScrollbar.draw(math.max(0, self.scroll - 1), #rows, LIST_ROWS)
end
...
function ManagerState:drawDetail()
  ...
  VRScrollbar.draw(math.max(0, self.descScroll - 1), #lines, visible)
  local rows = self:rowsForScreen()
  local y = 11
...
function ManagerState:draw()
  if self.screen == "options" then
    ...
    VRScrollbar.draw(self.scroll or 0, #(self.optionRows or {}),
                     OptionRows.VISIBLE)
    love.graphics.setColor(1, 1, 1, 1)
