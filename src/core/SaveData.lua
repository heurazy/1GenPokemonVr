...
function SaveData.defaultOptions()
  return {
    ...
    fpsCap = 60,
    -- VR presentation.  "immersive" is the original camera shipped by the
    -- OpenXR branch, so existing installs keep exactly their old framing.
    vrCamera = "immersive",
    vrWorldScale = 64,
    vrCameraDistance = 100,
    vrCameraPitch = 0,
    vrCameraHeight = 0,
    vrCameraOffsetX = 0,
    vrCameraOffsetZ = 0,
    vrHeadTracking = 100,
    -- Quest first-person calibration. Height is the virtual eye height in
    -- centimetres; render scale is the per-eye voxel resolution percentage.
    vrPlayerHeight = 130,
    vrRenderScale = 75,
    vrRenderDistance = 28,
    -- Preferred headset presentation rate. The native OpenXR bridge snaps
    -- this to a frequency actually enumerated by the connected runtime.
    vrRefreshRate = 90,
    -- Quest keeps shadows off by default to avoid the extra per-eye decal
    -- pass. The expensive dynamic shadow map is already unavailable there.
    vrShadows = false,
    vrTurnMode = "snap",
    vrSnapAngle = 30,
