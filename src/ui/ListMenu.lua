...
function ListMenu:onVRPointer(x, y)
  local row = math.floor((y - 16) / 16) + 1
  if row >= 1 and row <= self.rows and self.items[self.scroll + row] then
    self.index = self.scroll + row
  end
  return true
end

function ListMenu:vrScroll(delta)
  ...
  self.index = math.max(self.scroll + 1,
                        math.min(self.scroll + self.rows, self.index))
end

function ListMenu:vrScrollInfo()
  return self.scroll > 0, self.scroll + self.rows < #self.items
end

function ListMenu:vrScrollState()
  return self.scroll, math.max(0, #self.items - self.rows)
end

function ListMenu:vrScrollMetrics()
  return self.scroll, #self.items, self.rows
end

function ListMenu:vrSetScroll(value)
...
function ListMenu:draw()
  ...
  VRScrollbar.draw(self.scroll, #self.items, self.rows)
  love.graphics.setColor(1, 1, 1, 1)
end
