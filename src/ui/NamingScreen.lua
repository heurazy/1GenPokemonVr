...
function NamingScreen:onVRPointer(x, y)
  local grid = self:grid()
  local row = math.floor((y - 40) / 16) + 1
  if row < 1 or row > #grid then return true end
  local col = math.floor((x + 8) / 16)
  col = math.max(1, math.min(#grid[row], col))
