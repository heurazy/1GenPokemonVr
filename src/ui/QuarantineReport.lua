...
function QuarantineReport:onVRPointer(_, _)
  return true
end

function QuarantineReport:vrScrollMetrics()
  return self.offset, #self.lines, VISIBLE
end

function QuarantineReport:vrSetScroll(value)
  self.offset = math.max(0, math.min(self:maxOffset(), tonumber(value) or 0))
end
...
function QuarantineReport:draw()
  ...
  VRScrollbar.draw(self.offset, #self.lines, VISIBLE)
