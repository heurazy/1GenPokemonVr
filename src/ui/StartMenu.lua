...
function StartMenu.new(game)
  ...
  -- VR settings belong beside the ordinary options in the pause menu, so
  -- headset users do not have to open a second long options list first.
  if require("src.vr.OpenXR").requested() then
    table.insert(items, { label = Strings("VR OPTION"), onSelect = function()
      Screens.push(game, "VROptionsMenu", { onCancel = reopen })
    end })
  end
