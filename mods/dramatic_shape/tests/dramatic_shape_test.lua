...
local MOD_PATH = os.getenv("DS_MOD_PATH") or "mods/dramatic_shape"
...
local engineLayoutRow = rowIndex(layoutMenu, "battleLayout")
if engineLayoutRow then
  T.check(engineLayoutRow,
  ...
  -- The standalone Quest fork predates the optional WideBattle compositor.
  -- Its only battle surface is already the 160x144 OG layout Dramatic Shape
  -- requires, so there is no engine row to hide or value to pin.  The
  -- synthetic-row checks above still cover the official >=0.1.37 contract.
  T.check(not engineLayoutRow,
    "the classic-only Quest engine exposes no inoperative BATTLE LAYOUT row")
  Battles.setting:setIndex(1, menuGame)
end
end
...
-- Standalone OpenXR does not emit LÖVE/SDL axis events.  Its raw axis is
-- stored beside the SDL stick and must win over the quantised GB directions
-- so free movement remains analog and camera-relative in the headset.
local Game = require("src.core.Game")
local savedInput = Game.input
Game.input = {
  vrMoveAxis = { x = 0.6, y = -0.8 },
  stickAxis = { x = 0, y = 0 },
  isDown = function() return false end,
}
local mx, mz = FirstPerson.moveVector()
T.check(near(mx, 0.6) and near(mz, 0.8),
  "first-person free move reads the raw OpenXR left stick")
...
-- Regression for the engine API rename from canCollisionWarp() to
-- onWarpArrivalCell(): a blocked first-person push must remain safe.
local pushState = {
  player = { cellX = 1, cellY = 1, facing = "down" },
  checkEdgeExit = function() return false end,
  checkLedgeHop = function() return false end,
  checkBoulderPush = function() return false end,
  onWarpArrivalCell = function() return true end,
}
local pushOk, pushResult = pcall(FreeMove._pushSpecials,
                                 pushState, "left", "tile")
T.check(pushOk and pushResult == false,
  "first-person wall pushes support the renamed warp guard")
...
T.check(Dex.frame.tex == nil,
  "and no HUD texture is attached until something is put on it")
T.check(not Dex._screenBackingVisible(Dex.frame),
  "the tablet aperture stays transparent even with no menu or dialog")
T.check(pcall(Dex.draw),
  "drawing headless is a clean no-op -- no meshes, no crash")
Dex.screen({}, 0, 0, 1, 1)
T.check(not Dex._screenBackingVisible(Dex.frame),
  "a live tablet removes the opaque black backing behind its HUD")
T.check(type(Dex.SCREEN_KEY_MAX) == "number"
        and Dex.SCREEN_KEY_MAX > 0 and Dex.SCREEN_KEY_MAX < 0.2,
  "the live screen keys only near-black pixels, preserving bright UI")
local voxelShader = run.loader.exports.DRAMATIC_SHAPE.lib
  .require("Voxel3D")._shaderSource(false)
T.check(voxelShader:find("alphaKeyOn > 0.5", 1, true) ~= nil
        and voxelShader:find("alphaKeyTexel", 1, true) ~= nil
        and voxelShader:find("if (neighbour <= alphaKeyMax) discard", 1, true) ~= nil,
  "the 3D screen shader removes flat black while preserving dark glyph edges")
Dex.clear()
...
local grassCamera = { eye = { 0, 12, 0 }, focus = { 0, 12, 1 } }
T.check(VS_.grassChunkVisible(grassCamera,
          { cx = 0, cz = -48, radius = 24 }, 0, 0, true),
  "Quest grass culling always keeps the sector around the player's feet")
T.check(VS_.grassChunkVisible(grassCamera,
          { cx = 0, cz = 128, radius = 32 }, 0, 0, true),
  "Quest grass culling keeps the shortened forward grass range")
T.check(not VS_.grassChunkVisible(grassCamera,
          { cx = 0, cz = -160, radius = 32 }, 0, 0, true),
  "Quest grass culling rejects distant sectors behind the headset")
T.check(not VS_.grassChunkVisible(grassCamera,
          { cx = 0, cz = 500, radius = 16 }, 0, 0, true),
  "Quest grass culling rejects decorative tufts beyond their useful range")
T.check(VS_.grassChunkVisible(grassCamera,
          { cx = 0, cz = 0, radius = 16 }, 0, 120, true),
  "connected-map offsets are included in the grass visibility test")
local CM_ = run.loader.exports.DRAMATIC_SHAPE.lib.require("ChunkMesher")
local grassFront = {
  { 0, 0, 5 }, { 8, 0, 5 }, { 8, 8, 5 }, { 0, 8, 5 }, shade = 1,
}
local grassBack = {
  { 8, 0, 3 }, { 0, 0, 3 }, { 0, 8, 3 }, { 8, 8, 3 }, shade = 0.68,
}
local grassCap = {
  { 0, 8, 3 }, { 8, 8, 3 }, { 8, 8, 5 }, { 0, 8, 5 }, shade = 1,
}
T.check(CM_._questGrassFace(grassFront)
        and not CM_._questGrassFace(grassBack)
        and not CM_._questGrassFace(grassCap),
  "Quest keeps one two-sided grass silhouette and drops duplicate backs/caps")
T.eq(CM_._quadDirection(grassFront), 5,
  "vertical geometry is classified by its outward compass face")
T.eq(CM_._quadDirection({
  { 0, 0, 0 }, { 8, 0, 0 }, { 8, 0, 8 }, { 0, 0, 8 }, shade = 1,
}), nil, "horizontal terrain stays in the always-visible base mesh")
T.check(not VS_.terrainFaceVisible(grassCamera, 5, true)
        and VS_.terrainFaceVisible(grassCamera, 6, true)
        and VS_.terrainFaceVisible(grassCamera, 1, true),
  "Quest rejects the wall group facing away from the headset gaze")
T.check(VS_.terrainFaceVisible(grassCamera, 5, false),
  "directional face culling stays disabled outside the Quest first-person pass")
T.eq(VRRig.QUEST_FAR, 28,
  "Quest defaults to a wider but still bounded mobile horizon")
T.check(VS_.detailVisible(grassCamera, 0, -48, true),
  "Quest detail culling keeps nearby NPCs in every direction")
T.check(VS_.detailVisible(grassCamera, 0, 180, true),
  "Quest detail culling keeps distant NPCs in front")
T.check(not VS_.detailVisible(grassCamera, 0, -180, true),
  "Quest detail culling rejects distant NPCs behind the headset")
T.check(not VS_.detailVisible(grassCamera, 0, 260, true),
  "Quest detail culling rejects billboards beyond their useful range")
local frontNeighbour = { ox = -32, oy = 160,
  map = { def = { width = 2, height = 2 } } }
local rearNeighbour = { ox = -32, oy = -320,
  map = { def = { width = 2, height = 2 } } }
T.check(VS_.neighborVisible(grassCamera, frontNeighbour, true),
  "Quest keeps a connected map in front of the headset")
T.check(not VS_.neighborVisible(grassCamera, rearNeighbour, true),
  "Quest skips a connected map wholly behind the headset")
T.check(VS_.neighborVisible(grassCamera, rearNeighbour, false),
  "desktop and non-first-person views keep every connected map")
...
-- ------- standalone Quest adapter policies
--
-- The Android bridge itself needs a headset, but the adapter's two fragile
-- seams are pure enough to pin here: HEAD MOTION must preserve physical IPD,
-- and a failed side-by-side composition must unwind the graphics state.
;(function()
  local lib = run.loader.exports.DRAMATIC_SHAPE.lib
  local QuestVR = lib.require("QuestVR")
  local ShadowMap = lib.require("ShadowMap")
  local Voxel3D = lib.require("Voxel3D")

  local q = { 0, 0, 0, 1 }
  local left = {
    position = { 0.068, 0.2, -0.4 }, orientation = q, headTracking = 0,
  }
  local right = {
    position = { 0.132, 0.2, -0.4 }, orientation = q, headTracking = 0,
  }
  local lp, rp = QuestVR._trackingPoses(left, right)
  local function near(a, b) return math.abs(a - b) < 1e-6 end
  T.check(near(lp.pos[1], -0.032) and near(rp.pos[1], 0.032),
    "HEAD MOTION 0% freezes the head centre without collapsing the IPD")
  T.check(near(rp.pos[1] - lp.pos[1], 0.064),
    "the physical eye separation survives every tracking scale")
  T.check(near((lp.pos[2] + rp.pos[2]) * 0.5, 0)
          and near((lp.pos[3] + rp.pos[3]) * 0.5, 0),
    "0% removes centre translation on every axis")

  left.headTracking, right.headTracking = 50, 50
  lp, rp = QuestVR._trackingPoses(left, right)
  T.check(near((lp.pos[1] + rp.pos[1]) * 0.5, 0.05)
          and near((lp.pos[2] + rp.pos[2]) * 0.5, 0.1)
          and near((lp.pos[3] + rp.pos[3]) * 0.5, -0.2),
    "HEAD MOTION 50% scales only the tracked centre")
  T.check(near(rp.pos[1] - lp.pos[1], 0.064),
    "50% tracking still leaves IPD untouched")
  ...
  local controllerOut = { pos = {}, quat = {} }
  T.eq(QuestVR._controllerPose(controller, controllerOut), controllerOut,
    "the Quest controller adapter fills its supplied official pose record")
  T.same(controllerOut,
    { pos = { 0.12, -0.34, 0.56 }, quat = { 0.1, 0.2, 0.3, 0.9 } },
    "the handheld Pokedex receives the tracked controller pose unchanged")
  T.eq(QuestVR._controllerPose({ active = false }, controllerOut), nil,
    "an untracked controller cannot leave a stale handheld Pokedex pose")

  -- The Quest render path mutates a stable camera graph instead of
  -- allocating matrices and vectors twice per headset frame. The optional
  -- buffers must preserve both the official arithmetic and every identity
  -- the hot path relies on.
  local VRRig = lib.require("VRRig")
  local pose = { pos = { 0.03, 0.04, -0.02 }, quat = q }
  local fov = {
    angleLeft = -0.8, angleRight = 0.75,
    angleUp = 0.72, angleDown = -0.7,
  }
  local pivot = { 160, 0, 144 }
  local anchor = { 0, -0.5, -0.75 }
  local baseline = VRRig.eyeCamera(pose, fov, pivot, anchor, 64, 0.2)
  local cached, scratch = {}, {}
  T.eq(VRRig.eyeCamera(pose, fov, pivot, anchor, 64, 0.2,
                       cached, scratch), cached,
    "the reusable eye-camera path returns the supplied record")
  local view, proj = cached.view, cached.proj
  local eye, focus, skyRay = cached.eye, cached.focus, cached.skyRay
  local base, du, dv = skyRay.base, skyRay.du, skyRay.dv
  T.same(cached, baseline,
    "the reusable eye-camera path keeps the official camera arithmetic")
  VRRig.eyeCamera(pose, fov, pivot, anchor, 64, 0.2, cached, scratch)
  T.check(cached.view == view and cached.proj == proj
          and cached.eye == eye and cached.focus == focus
          and cached.skyRay == skyRay and cached.skyRay.base == base
          and cached.skyRay.du == du and cached.skyRay.dv == dv,
    "a second eye-camera update reuses every matrix and vector table")

  local pivotOut, anchorOut = {}, {}
  T.eq(VRRig.dioramaPivot(12, 34, pivotOut), pivotOut,
    "the Quest pivot helper fills its supplied table")
  T.eq(VRRig.dioramaAnchor(math.rad(35), 0.1, anchorOut), anchorOut,
    "the Quest anchor helper fills its supplied table")

  T.eq(ShadowMap._mobileShadowFallback("iOS", false), true,
    "iOS keeps the official decal fallback")
  T.eq(ShadowMap._mobileShadowFallback("Android", false), false,
    "generic Android keeps the official shadow-map capability probe")
  T.eq(ShadowMap._mobileShadowFallback("Android", true), true,
    "only standalone Quest forces the cheaper decal shadows")

  T.eq(Voxel3D._readableDepthAllowed("Android", true), false,
    "standalone Quest avoids fragile sampled depth textures")
  T.eq(Voxel3D._readableDepthAllowed("Android", false), true,
    "generic Android retains the readable-depth capability probe")
  T.eq(Voxel3D._readableDepthAllowed("Windows", true), true,
    "desktop VR keeps readable depth and full water reflections")

  local smallW, smallH = QuestVR._renderDimensions(1000, 500, 0.60)
  T.eq(smallW, 600, "Quest safe scale reduces the eye width")
  T.eq(smallH, 300, "Quest safe scale reduces the eye height")
  T.check(near(QuestVR._firstPersonEyeHeight(130, 10), 13),
    "the default Quest player height keeps the official 1.30 m eye line")
  T.check(near(QuestVR._firstPersonEyeHeight(170, 5), 8.5),
    "player height remains 1.70 m when WORLD SIZE changes the px-per-metre scale")
  local cappedW, cappedH = QuestVR._renderDimensions(4000, 3000, 1.0)
  T.check(cappedW * cappedH <= QuestVR.MAX_EYE_PIXELS,
    "oversized Quest eye buffers are capped below the GPU memory budget")
  T.check(math.abs(cappedW / cappedH - 4 / 3) < 0.01,
    "the Quest eye-buffer cap preserves aspect ratio")

  local realGraphics = love.graphics
  local resetCalls = {}
  love.graphics = {
    setShader = function() resetCalls.shader = true end,
    setDepthMode = function() resetCalls.depth = true end,
    setMeshCullMode = function(mode) resetCalls.cull = mode end,
    setCanvas = function() resetCalls.canvas = true end,
  }
  local abortOK = pcall(Voxel3D.abortScene, false)
  love.graphics = realGraphics
  T.eq(abortOK, true, "a failed eye can always abort its partial 3D scene")
  T.check(resetCalls.shader and resetCalls.depth and resetCalls.canvas
          and resetCalls.cull == "none",
    "scene abort restores shader, depth, cull and framebuffer state")
  ...
  love.graphics = {
    getCanvas = function() return previous end,
    push = function() end,
    setCanvas = function(canvas) current = canvas end,
    clear = function() end,
    setColor = function() end,
    draw = function() error("forced Quest composition failure") end,
    pop = function() popped = true end,
  }
  local callOK, composeOK = pcall(QuestVR._composeEyes,
                                  {}, { fakeEye, fakeEye }, 64, 64)
  love.graphics = realGraphics
  T.eq(callOK, true, "a composition failure is contained")
  T.eq(composeOK, false, "the failed composition reports fallback")
  T.check(popped and current == previous,
    "the graphics stack and previous canvas are restored after failure")
end)()
