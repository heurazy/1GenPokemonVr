...
local SHADER = [[
  varying LOVE_HIGHP_OR_MEDIUMP float vShade;
  // Large shadow coordinates need high precision on mobile GLES. Without
  // it, the two triangles of a voxel face can quantise differently and leave
  // a dark diagonal seam on Quest GPUs.
  ...
  uniform float alphaKeyOn;   // 1 only for the live transparent VR tablet
  uniform float alphaKeyMax;  // darkest source value still treated as empty
  uniform vec2 alphaKeyTexel; // one source pixel, for preserving dark glyphs
  ...
  vec4 effect(vec4 color, Image tex, vec2 tc, vec2 sc) {
    ...
    if (p.a < 0.5) discard;
    // The live Pokedex screen behaves like a phone-camera HUD: its black
    // backdrop is a hole onto the world while the brighter text, icons and
    // meters remain real depth-tested pixels. A hard key is deliberate --
    // the source is nearest-neighbour pixel art, so partial alpha would only
    // write fuzzy transparent fringes into the depth buffer.
    if (alphaKeyOn > 0.5
        && max(max(p.r, p.g), p.b) <= alphaKeyMax) {
      // Keep black text and icon outlines: those thin strokes touch brighter
      // neighbours. Only a black pixel surrounded by more black belongs to
      // the large opaque backdrop and becomes the camera-like opening.
      vec3 l = Texel(tex, tc - vec2(alphaKeyTexel.x, 0.0)).rgb;
      vec3 r = Texel(tex, tc + vec2(alphaKeyTexel.x, 0.0)).rgb;
      vec3 u = Texel(tex, tc - vec2(0.0, alphaKeyTexel.y)).rgb;
      vec3 d = Texel(tex, tc + vec2(0.0, alphaKeyTexel.y)).rgb;
      float neighbour = max(max(max(l.r, l.g), l.b),
                        max(max(max(r.r, r.g), r.b),
                        max(max(max(u.r, u.g), u.b),
                            max(max(d.r, d.g), d.b))));
      if (neighbour <= alphaKeyMax) discard;
    }
...
-- Readable depth textures are optional and considerably more fragile than an
-- ordinary depth attachment on mobile GLES drivers. The Quest profile only
-- enables SKY water (no screen-space depth march), so allocating two sampled
-- depth textures buys nothing there and has caused native driver exits during
-- the first stereo frame. Keep the normal capability probe everywhere else.
local function readableDepthAllowed(osName, questBuild)
  return not (osName == "Android" and questBuild)
end

local function readableDepthEnabled()
  local osName = love.system and love.system.getOS and love.system.getOS()
                 or "Unknown"
  local quest = false
  local ok, OpenXR = pcall(require, "src.vr.OpenXR")
  if ok and OpenXR and OpenXR.isQuestBuild then
    quest = OpenXR.isQuestBuild() and true or false
  end
  return readableDepthAllowed(osName, quest)
end

local function newDepth(w, h)
  if not readableDepthEnabled() then return nil end
...
function Voxel3D.beginScene(w, h, cx, cy, vw, vh, sky, slot)
  ...
  if not ok then
    ...
    -- A canvas that cannot be attached must not remain cached. Keeping it
    -- makes every later frame retry the same invalid framebuffer and, in
    -- stereo, can leave one eye valid while the other is permanently nil.
  ...
  -- ordinary world geometry is never chroma-keyed. The Pokedex brackets its
  -- live screen draw explicitly, and this reset prevents a failed prop draw
  -- from making the next frame's dark terrain disappear.
  pcall(sh.send, sh, "alphaKeyOn", 0)
  pcall(sh.send, sh, "alphaKeyMax", 0.08)
  pcall(sh.send, sh, "alphaKeyTexel", { 0, 0 })
...
-- `copyColor=false` keeps the mirror as a valid bound texture but skips the
-- full-frame colour copy; SKY-only water needs depth occlusion and sky data,
-- not the reflected scene.
--
...
function Voxel3D.beginWater(paint, copyColor)
  ...
  if copyColor == nil then copyColor = true end
  ...
  if copyColor and paint and activeShader then
...
-- Key the darkest texels out of the next draw. This is intentionally a
-- per-draw scene-shader toggle rather than a special 2D canvas pass: the live
-- tablet still needs the exact same stereo projection, hand tracking and
-- depth test as its red body. `threshold` is normalized RGB (0..1).
function Voxel3D.alphaKey(on, threshold, texelX, texelY)
  if not (active and activeShader) then return end
  pcall(activeShader.send, activeShader, "alphaKeyMax",
        math.max(0, math.min(1, tonumber(threshold) or 0.08)))
  pcall(activeShader.send, activeShader, "alphaKeyTexel",
        { math.max(0, tonumber(texelX) or 0),
          math.max(0, tonumber(texelY) or 0) })
  pcall(activeShader.send, activeShader, "alphaKeyOn", on and 1 or 0)
end
...
-- Abort a half-drawn scene after a protected render failed. A Lua error in
-- one eye used to escape through QuestVR's pcall while leaving the canvas,
-- depth mode, shader and cull mode bound. The next frame then rendered into a
-- stale framebuffer and could take the Android GL driver down with it.
function Voxel3D.abortScene(discardSlot)
  -- `held` deliberately survives endScene so helpers can inspect the last
  -- completed target. Only discard it when a pass was genuinely active at
  -- the time of the failure. A normal "terrain is still warming" return has
  -- no active pass and must never evict the previously completed right eye.
  ...
  if discardSlot and wasActive and held then
    for name, slotHeld in pairs(slots) do
      if slotHeld == held then
        releaseSlot(slotHeld)
        slots[name] = nil
        break
      end
...
-- Pure seams for the mobile policy and shader-regression tests.
Voxel3D._readableDepthAllowed = readableDepthAllowed
Voxel3D._shaderSource = function(grid)
