...
local function mobileShadowFallback(osName, questBuild)
  return osName == "iOS" or (osName == "Android" and questBuild)
end
...
-- Whether shadows are wanted at all. Desktop keeps the upstream behaviour;
-- Quest reads the lightweight VR option so both the dynamic map and the
-- per-eye fallback decals can be skipped together.
function ShadowMap.enabled()
  if OpenXR and OpenXR.shadowsEnabled then
    return OpenXR.shadowsEnabled()
  end
  return true
end
...
function ShadowMap.available()
  if not ShadowMap.enabled() then return false end
  if love.system and love.system.getOS then
    local os = love.system.getOS()
    -- The standalone Quest profile uses the cheap decal fallback. A second
    -- full terrain pass plus a large readable depth target costs too much on
    -- tile-based mobile GPUs and has shown diagonal precision artefacts on
    -- several Meta firmware/driver combinations.
    local quest = OpenXR and OpenXR.isQuestBuild
                  and OpenXR.isQuestBuild() or false
    if mobileShadowFallback(os, quest) then return false end
  end
...
-- Pure policy seam for the ROM-free suite: generic Android keeps upstream's
-- capability probe, while the standalone Quest build takes the cheap path.
ShadowMap._mobileShadowFallback = mobileShadowFallback
