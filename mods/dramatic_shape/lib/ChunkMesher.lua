...
-- Standalone Quest splits the exceptionally dense tall-grass mesh into
-- world-space sectors. Each eye then draws only the sectors close to the
-- player or inside the headset's view cone instead of submitting the whole
-- route twice. Desktop keeps the historical single mesh and draw call.
ChunkMesher.GRASS_CHUNK_SIZE = 96
...
-- Dominant outward compass direction of a vertical quad. Horizontal and
-- sloped surfaces return nil and remain in the always-visible base mesh.
local function quadDirection(c)
  if not (c and c[1] and c[2] and c[3]) then return nil end
  ...
  if any >= anx and any >= anz then return nil end
  if anx >= anz then return nx >= 0 and 1 or 2 end
  return nz >= 0 and 5 or 6
end

ChunkMesher._quadDirection = quadDirection
...
local function runGeometry(map, bodyOnly, masks, sink, waterSink, faceSinks)
  local push = sink.push
  local waterPush = waterSink and waterSink.push or nil
  local function facePush(d, c, uv, shade)
    local target = faceSinks and faceSinks[d]
    return (target and target.push or push)(c, uv, shade)
  end
  ...
  for _, q in ipairs(S.objectQuads) do
    ...
    if q.own or outwardOnEdge(q, x0, z0, x1, z1)
      ...
      facePush(quadDirection(c), c, quadUV(q), groundShades(q, q.shade))
    end
  end
  ...
  for _, st in ipairs(S.roundStamps or {}) do
    ...
    if not skipAll then
      for _, q in ipairs(st.quads) do
        ...
        if ok then
          facePush(quadDirection(sc), sc, quadUV(q), groundShades(sc, q.shade))
        end
      end
    end
  end
...
local TERRAIN_FACE_DIRS = { 1, 2, 5, 6 }

local function newTerrainFaceSinks()
  if not questBuild() then return nil end
  return {
    [1] = newSink(), [2] = newSink(),
    [5] = newSink(), [6] = newSink(),
  }
...
local function finishTerrain(sink, faceSinks)
  local base = sink.finish()
  if not faceSinks then return base end
  local faces, any = {}, false
  for _, d in ipairs(TERRAIN_FACE_DIRS) do
    local mesh = faceSinks[d].finish()
    faces[d] = mesh
    any = any or mesh ~= nil
  end
  if not any then return base end
  local batch = { base = base, faces = faces, terrainBatch = true }
  function batch:release()
    if self.base and self.base.release then pcall(self.base.release, self.base) end
    self.base = nil
    for _, d in ipairs(TERRAIN_FACE_DIRS) do
      local mesh = self.faces and self.faces[d]
      if mesh and mesh.release then pcall(mesh.release, mesh) end
      if self.faces then self.faces[d] = nil end
    end
  end
  return batch
end
...
function ChunkMesher.build(map, bodyOnly, masks, split)
  ...
  local faceSinks = newTerrainFaceSinks()
  runGeometry(map, bodyOnly, masks, sink, waterSink, faceSinks)
  return finishTerrain(sink, faceSinks),
         waterSink and waterSink.finish() or nil
end
...
-- Quest renders grass with mesh culling disabled, so one vertical face is
-- already visible from both sides. The source geometry also carries a darker
-- duplicate back face and a tiny horizontal cap for every blade run; dropping
-- those two redundant surfaces cuts most grass vertices and overdraw without
-- changing the visible silhouette.
local function questGrassFace(q)
  if not (q and q[1] and q[2] and q[3] and q[4]) then return false end
  if (tonumber(q.shade) or 1) < 0.99 then return false end
  local z = q[1][3]
  local y = q[1][2]
  local variesY = false
  for i = 2, 4 do
    if math.abs((q[i][3] or z) - z) > 1e-6 then return false end
    if math.abs((q[i][2] or y) - y) > 1e-6 then variesY = true end
  end
  return variesY
end

ChunkMesher._questGrassFace = questGrassFace

-- Build a Quest-only batch of grass sectors. Bounds are kept beside each
-- mesh so VoxelScene can reject a sector before issuing a draw call. The
-- sectors use map-local world coordinates; connected-map offsets are applied
-- by the caller exactly as they are for the terrain mesh.
local function grassChunks(quads, chunkSize)
  if #quads == 0 then return nil end
  chunkSize = chunkSize or ChunkMesher.GRASS_CHUNK_SIZE
  local groups, order = {}, {}
  for _, q in ipairs(quads) do
    if questGrassFace(q) then
    local cx, cz = 0, 0
    for i = 1, 4 do
      cx = cx + q[i][1]
      cz = cz + q[i][3]
    end
    cx, cz = cx * 0.25, cz * 0.25
    local gx, gz = math.floor(cx / chunkSize), math.floor(cz / chunkSize)
    local key = gx .. ":" .. gz
    local g = groups[key]
    if not g then
      g = { quads = {}, x0 = math.huge, z0 = math.huge,
            x1 = -math.huge, z1 = -math.huge, key = key }
      groups[key] = g
      order[#order + 1] = g
    end
    g.quads[#g.quads + 1] = q
  ...
  local batch = { chunkSize = chunkSize }
  for _, g in ipairs(order) do
    local mesh = quadsMesh(g.quads)
    if mesh then
      local dx, dz = g.x1 - g.x0, g.z1 - g.z0
      batch[#batch + 1] = {
        mesh = mesh,
        x0 = g.x0, z0 = g.z0, x1 = g.x1, z1 = g.z1,
        cx = (g.x0 + g.x1) * 0.5,
        cz = (g.z0 + g.z1) * 0.5,
        radius = math.sqrt(dx * dx + dz * dz) * 0.5,
      }
    end
  end
  function batch:release()
    for _, chunk in ipairs(self) do
      if chunk.mesh and chunk.mesh.release then
        pcall(chunk.mesh.release, chunk.mesh)
      end
      chunk.mesh = nil
    end
  end
...
-- The tall-grass rows as their own drawable: VoxelScene draws it AFTER the
-- characters so the southern row of a grass cell still overdraws a walker's
-- feet. Quest receives a cullable sector batch; desktop keeps one mesh.
local function buildGrassMesh(map)
  local quads = Structures.forMap(map).grassQuads
  if questBuild() then return grassChunks(quads) end
...
local function runJob(job)
  ...
  local faceSinks = newTerrainFaceSinks()
  runGeometry(map, job.slot == "body", job.masks, sink, waterSink, faceSinks)
  local mesh = finishTerrain(sink, faceSinks)
...
-- Iterate every GPU mesh carried by a terrain drawable. Desktop terrain is
-- one mesh; Quest terrain keeps horizontal/sloped/object geometry in `base`
-- and four directional wall groups so the eye pass can skip faces pointing
-- away from the headset before submitting any vertices.
function ChunkMesher.eachTerrainMesh(terrain, fn)
  if not (terrain and fn) then return end
  if not terrain.terrainBatch then
    fn(terrain, nil)
    return
  end
  if terrain.base then fn(terrain.base, nil) end
  for _, d in ipairs(TERRAIN_FACE_DIRS) do
    local mesh = terrain.faces and terrain.faces[d]
    if mesh then fn(mesh, d) end
  end
end

function ChunkMesher.drawTerrain(terrain, texture, model,
                                 visible, camera, enabled)
  local draws = 0
  ChunkMesher.eachTerrainMesh(terrain, function(mesh, d)
    if d == nil or not visible or visible(camera, d, enabled) then
...
-- Draw either the historical single grass mesh or Quest's sector batch.
-- `visible(chunk)` may reject sectors before any GPU work is submitted.
function ChunkMesher.drawGrass(map, texture, model, pull,
                               visible, camera, ox, oz, enabled)
  local grass = ChunkMesher.grass(map)
  if not grass then return 0 end
  if grass.setTexture then
    Voxel3D.draw(grass, texture, model, pull)
    return 1
  end
  local draws = 0
  for _, chunk in ipairs(grass) do
    if chunk.mesh and (not visible
        or visible(camera, chunk, ox, oz, enabled)) then
      Voxel3D.draw(chunk.mesh, texture, model, pull)
      draws = draws + 1
    end
  end
  return draws
