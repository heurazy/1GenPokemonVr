...
local function vrOn()
  ...
  if ok and vr and vr.enabled and vr.enabled() then return true end
  local okQuest, quest = pcall(V.require, "QuestVR")
  return okQuest and quest and quest.supported and quest.supported() or false
end
...
function OverworldBattle.update(dt)
  ...
  local okVR, vrOn = pcall(function()
    ...
    if vr.active and vr.active() then return true end
    local quest = V.require("QuestVR")
    -- update runs before OpenXR.beginFrame, so QuestVR.active() is false at
    -- this point. The standalone build itself is the durable signal that the
    -- upcoming draw will consume the animation texture in stereo.
    return quest.supported and quest.supported() or false
  end)
