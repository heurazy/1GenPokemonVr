...
local function pushSpecials(state, dir, why)
  ...
  -- Collision-warp eligibility moved from canCollisionWarp() to the
  -- warp-arrival-cell guard in the engine. Keep the old hook as a fallback
  -- for older engine revisions, but never assume either method exists: a
  -- missing optional hook must not tear down the whole LÖVE update loop.
  local warpAllowed = true
  if state.onWarpArrivalCell then
    warpAllowed = not state:onWarpArrivalCell()
  elseif state.canCollisionWarp then
    warpAllowed = state:canCollisionWarp()
  end
  if why ~= "entity" and warpAllowed then
    local Game = require("src.core.Game")
...
FreeMove._pushSpecials = pushSpecials -- named for the regression suite
