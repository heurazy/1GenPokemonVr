...
function FirstPerson.moveVector()
  ...
  -- OpenXR bypasses LÖVE's SDL gamepad events, so its left stick has a raw
  -- pair of its own.  Prefer whichever physical stick is actually farther
  -- from centre; falling through to quantised direction bits remains useful
  -- for keyboard, d-pad and older bridges.
  local vrAxis = input and input.vrMoveAxis or nil
  local padAxis = input and input.stickAxis or nil
  local ax = vrAxis
  if padAxis and (not ax or
      padAxis.x * padAxis.x + padAxis.y * padAxis.y >
      ax.x * ax.x + ax.y * ax.y) then
    ax = padAxis
  end
  if ax then
